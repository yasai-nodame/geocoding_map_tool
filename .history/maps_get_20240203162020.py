...
def get_nearby_places(address,radius_km=10):
    ...
    if response.status_code == 200:
        ...
        int_latitude = float(latitude) #floatにキャッチする
        
        for i in range(10):
            
            #微小に座標を変化させる
            offset_lat = (2 * (0.5 - i / 10)) * (radius_km / 110.574) #1度あたり約110.574km
            offset_lon = (2 * (0.5 - i / 10)) * (radius_km / (111.32 * abs(int_latitude))) #緯度による経度の変化
            
            current_lat = int_latitude + offset_lat 
            current_lon = longitude + offset_lon 
            ...
            response = requests.get(nominatim_url,params=reverse_params)
            print(response.json())
